...
func formatModules(ms []Module) string {
	...
	for _, m := range ms {
		...
		b.WriteString(fmt.Sprintf(
			"\t{ID: %q, Name: %q, Version: %q, Status: %q, URL: %q, IconURL: %q, LatestVersion: %q, LateCommits: %d, LastUpdate: time.Date(%d, %d, %d, %d, %d, %d, 0, time.UTC)},\n",
			m.ID, m.Name, m.Version, m.Status, m.URL, m.IconURL, m.LatestVersion, m.LateCommits,
