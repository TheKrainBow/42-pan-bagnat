...
func ComposeRebuild(module Module) error {
    ...
    cmdBuild.Dir = dir
    if err := runAndLog(module.ID, cmdBuild); err != nil { return err }
    ...
    cmdUp.Dir = dir
    if err := runAndLog(module.ID, cmdUp); err != nil { return err }
    notifyContainersChanged(module)
    return nil
}
...
func ComposeDown(module Module) error {
    ...
    cmd.Dir = dir
    if err := runAndLog(module.ID, cmd); err != nil { return err }
    notifyContainersChanged(module)
    return nil
}
...
func RemoveContainer(name string) error {
    if strings.TrimSpace(name) == "" { return fmt.Errorf("empty container name") }
...
func GetAllContainers() ([]AllContainer, error) {
    ...
    if err != nil { return nil, fmt.Errorf("failed to load modules: %w", err) }
    modBySlug := make(map[string]Module)
    for _, m := range mods { modBySlug[m.Slug] = m }

    baseRepoPath := os.Getenv("REPO_BASE_PATH")
    if baseRepoPath == "" { baseRepoPath = "../../repos" }

    type composeInfo struct{ Services []string; Networks []string }
    ...
    for _, m := range mods {
        ...
        networks = networks[:0]
        for n := range netsSet { networks = append(networks, n) }
    ...
    queue := []string{}
    for _, ci := range expects { for _, n := range ci.Networks { if !visitedNet[n] { visitedNet[n] = true; queue = append(queue, n) } } }
    ...
    parseLine := func(line string) (name, rawStatus, project string, nets []string) {
        ...
        name = parts[0]; rawStatus = parts[1]; project = parts[2];
        if parts[3] != "" { nets = strings.Split(parts[3], ",") }
        return
    }
    // convert status
    ...
    for i := 0; i < len(queue); i++ {
        ...
        var out bytes.Buffer
        cmd.Stdout = &out
        if err := cmd.Run(); err != nil { continue }
        ...
        for _, line := range strings.Split(text, "\n") {
            ...
            if !ok {
                skip := true
                for _, n := range nets { if n != "pan-bagnat-net" { skip = false; break } }
                if skip { continue }
                ...
                for _, n := range nets { if !visitedNet[n] { visitedNet[n] = true; queue = append(queue, n) } }
                continue
            }
            st, reason, since := toStatus(raw)
    ...
    for slug, ci := range expects {
        for _, svc := range ci.Services {
            ...
            if _, ok := containers[expectedName]; ok { continue }
            m := modBySlug[slug]
    ...
    for _, v := range containers { out = append(out, v) }
    return out, nil
}
