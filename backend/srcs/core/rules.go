...
func evalRuleTrace(rule any, ctx any) TraceNode {
    ...
    case "array":
        ...
        var countPtr *int
        if c := toIntPtr(m["count"]); c != nil { countPtr = c }
        var idxPtr *int
        if i := toIntPtr(m["index"]); i != nil { idxPtr = i }
        ...
        for i := 0; i < size; i++ {
            ...
            if t.Result { matches++; matchedIdx = append(matchedIdx, i) }
            kids = append(kids, t)
        }
        res := false
        switch quant {
        ...
        if !res {
            ...
            case "COUNT_GTE":
                if countPtr != nil { msg = fmt.Sprintf("matched %d < required %d", matches, *countPtr) }
            case "COUNT_EQ":
                if countPtr != nil { msg = fmt.Sprintf("matched %d != required %d", matches, *countPtr) }
            case "COUNT_LTE":
                if countPtr != nil { msg = fmt.Sprintf("matched %d > allowed %d", matches, *countPtr) }
            case "INDEX":
                if idxPtr == nil || *idxPtr < 0 || *idxPtr >= size {
