...
func GitListCommitsRef(module Module, ref string, limit int) ([]GitCommit, error) {
	...
	if remoteRef != "" {
		...
		if len(parts) == 2 {
			_ = runAndLog(module.ID, exec.Command("git", "-C", repoDir, "fetch", parts[0], parts[1]))
		} else {
			_ = runAndLog(module.ID, exec.Command("git", "-C", repoDir, "fetch", "--all", "--prune"))
		}
	} else {
		_ = runAndLog(module.ID, exec.Command("git", "-C", repoDir, "fetch", "--all", "--prune"))
	}
...
func GitCheckout(module Module, ref string) error {
	...
	if e := runAndLog(module.ID, exec.Command("git", "-C", repoDir, "fetch", "--all", "--prune")); e != nil {
		return e
	}
