...
func DeleteModuleRepoDir(module Module) error {
	targetDir, err := ModuleRepoPath(module)
	if err != nil {
		return fmt.Errorf("invalid module slug: %w", err)
	}
...
func tempSSHForModule(module Module) (string, func(), error) {
	...
	// Build a safer SSH command by default: enforce host key checking and allow a custom known_hosts
	sshStrict := strings.ToLower(strings.TrimSpace(os.Getenv("GIT_SSH_STRICT")))
	// strict by default unless explicitly disabled ("0" or "false")
	strict := !(sshStrict == "0" || sshStrict == "false" || sshStrict == "off")
	knownHosts := strings.TrimSpace(os.Getenv("GIT_SSH_KNOWN_HOSTS"))

	parts := []string{"ssh", "-i", tmpKey.Name(), "-o", "IdentitiesOnly=yes"}
	if strict {
