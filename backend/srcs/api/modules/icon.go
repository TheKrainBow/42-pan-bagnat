...
func detectImageExt(data []byte) string {
    ...
    default:
        ...
        if strings.Contains(ct, "png") { return ".png" }
        if strings.Contains(ct, "jpeg") { return ".jpg" }
        if strings.Contains(ct, "webp") { return ".webp" }
        if strings.Contains(ct, "gif") { return ".gif" }
        return ".png"
    }
}
...
func saveModuleIcon(moduleID string, data []byte, hintedName string) (string, error) {
    ...
    if err := os.WriteFile(dst, data, 0o644); err != nil { return "", err }
    // public URL
...
func savePageIcon(pageID string, data []byte, hintedName string) (string, error) {
    ...
    if err != nil { return "", err }
    ext := filepath.Ext(hintedName)
    if ext == "" || len(ext) > 5 { ext = detectImageExt(data); if ext == "" { ext = ".png" } }
    if mt := mime.TypeByExtension(ext); mt == "" { ext = detectImageExt(data) }
...
func SetModuleIconUpload(w http.ResponseWriter, r *http.Request) {
    ...
    if moduleID == "" { http.Error(w, "missing moduleID", http.StatusBadRequest); return }
    ...
    if err != nil { http.Error(w, "failed to read file", http.StatusBadRequest); return }

    url, err := saveModuleIcon(moduleID, data, hdr.Filename)
    if err != nil { http.Error(w, "failed to save icon", http.StatusInternalServerError); return }

    // Patch DB
    if _, err := core.PatchModule(core.ModulePatch{ ID: moduleID, IconURL: &url }); err != nil {
        http.Error(w, "failed to update module", http.StatusInternalServerError); return
    }
...
func SetModuleIconFromURL(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil || resp.StatusCode != 200 { http.Error(w, "failed to download", http.StatusBadRequest); return }
    defer resp.Body.Close()
    data, err := io.ReadAll(resp.Body)
    if err != nil { http.Error(w, "failed to read image", http.StatusBadRequest); return }
...
func SetModuleIconFromRepo(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { http.Error(w, "failed to read file from repo", http.StatusBadRequest); return }
...
func SetPageIconUpload(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil { log.Printf("page icon upload: missing file: %v", err); http.Error(w, "missing file", http.StatusBadRequest); return }
    defer file.Close()
    data, err := io.ReadAll(file)
    if err != nil { log.Printf("page icon upload: read error: %v", err); http.Error(w, "failed to read", http.StatusBadRequest); return }
    url, err := savePageIcon(pageID, data, "upload")
    if err != nil { log.Printf("page icon upload: save error: %v", err); http.Error(w, "failed to save", http.StatusInternalServerError); return }
    if _, err := database.PatchModulePage(database.ModulePagePatch{ ID: pageID, IconURL: &url }); err != nil {
        ...
        http.Error(w, "failed to update page", http.StatusInternalServerError); return
    }
    log.Printf("[page-icon][upload] OK moduleID=%s pageID=%s icon_url=%s (size=%d bytes)", moduleID, pageID, url, len(data))
    w.Header().Set("Content-Type", "application/json"); json.NewEncoder(w).Encode(map[string]string{"icon_url": url})
}
...
func SetPageIconFromURL(w http.ResponseWriter, r *http.Request) {
    ...
    defer resp.Body.Close()
    data, err := io.ReadAll(resp.Body)
    if err != nil { log.Printf("page icon url: read body failed: %v", err); http.Error(w, "failed to read", http.StatusBadRequest); return }
    url, err := savePageIcon(pageID, data, filepath.Base(body.URL))
    if err != nil { log.Printf("page icon url: save error: %v", err); http.Error(w, "failed to save", http.StatusInternalServerError); return }
    if _, err := database.PatchModulePage(database.ModulePagePatch{ ID: pageID, IconURL: &url }); err != nil {
...
func SetPageIconFromRepo(w http.ResponseWriter, r *http.Request) {
    ...
    dbm, err := database.GetModule(moduleID)
    if err != nil { log.Printf("page icon repo: module not found: %v", err); http.Error(w, "module not found", http.StatusNotFound); return }
    m := core.DatabaseModuleToModule(dbm)
    data, err := core.ReadModuleFile(m, body.Path)
    if err != nil { log.Printf("page icon repo: read from repo failed: %v", err); http.Error(w, "failed to read from repo", http.StatusBadRequest); return }
    url, err := savePageIcon(pageID, data, filepath.Base(body.Path))
    if err != nil { log.Printf("page icon repo: save error: %v", err); http.Error(w, "failed to save", http.StatusInternalServerError); return }
    if _, err := database.PatchModulePage(database.ModulePagePatch{ ID: pageID, IconURL: &url }); err != nil {
...
func SetPageIconClear(w http.ResponseWriter, r *http.Request) {
    ...
    if strings.TrimSpace(pageID) == "" { http.Error(w, "missing pageID", http.StatusBadRequest); return }
    if err := database.SetPageIconURL(pageID, nil); err != nil {
        log.Printf("page icon clear: DB error: %v", err)
        http.Error(w, "failed to clear icon", http.StatusInternalServerError); return
    }
