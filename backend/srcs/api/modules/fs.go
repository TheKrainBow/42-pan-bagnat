...
func WriteFsFile(w http.ResponseWriter, r *http.Request) {
	...
	if err := core.WriteModuleFile(mod, body.Path, []byte(body.Content)); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func DeleteFsPath(w http.ResponseWriter, r *http.Request) {
	...
	if err := core.DeleteModulePath(mod, body.Path); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func MkdirFsPath(w http.ResponseWriter, r *http.Request) {
	...
	if err := core.MkdirModule(mod, body.Path); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	w.WriteHeader(http.StatusNoContent)
